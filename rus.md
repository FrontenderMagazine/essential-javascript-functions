Я помню времена, когда JavaScript только появился, и нам нужно было
писать простые функции почти для всего, потому что производители
браузеров реализовывали их по-разному. Это относилось не только к
специфичным функциям, но и к основным тоже, таким как
```addEventListener```, ```attachEvent```.
Сейчас всё изменились, но всё есть еще несколько функций 
для выполнения простых рутиных задач, которые каждый
разработчик должен иметь в своем арсенале.

### [```debounce```][1]

Функция ```debounce``` может стать незаменимой когда дело касается до
...
может сделать ваш код более производительным:
...
Функция ```debounce``` не позволит обратному вызову исполняться чаще
чем один раз в определенный период времени. Это особенно важно 
...
Иногда вам нужно, чтобы функция выполниласьтолько один раз, как вы 
бы использовали событие ```onload```. Этот код даёт вам такую
возможность:
...
Получить абсолютный URL из переменной строки не так просто как кажется.
...
Знание является ли функция нативной может быть необходимым, если вы
...
Это будет особенно полезно, когда вы работаете с динамическим,
AJAX-нагруженным сайтом.
Если вы добавите стили для конкретного селектора, вам не придется
отдельно добавлять стили для элементов, которые могут соответствовать
этому селектору сейчас или в будущем.

### [`matchesSelector`][7]

Как правило, мы проверям полученные данные, прежде чем продолжать:
удостверяемся, что значение верно, что содержимое формы валидно, и т.д. Но как часто мы проверяем, что элемент подходит для дальнейших действий? Для проверки, что элемент соответствует селектору, вы можете использовать функцию ```matchesSelector```:
